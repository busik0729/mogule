...
export class FuseNavigationService
{
     ...
     * Constructor
     */
    constructor(private storage: LocalStorageService, private _auth: AuthService)
    {
        // Set the defaults
    ...
    hiddenItems() {
        ...
        let userRole = "Гость";
        if (user != undefined) {
            userRole = user.role_string;
        }

        navigation.forEach((v) => {
            this.proccessHiddenItems(v, userRole);
        });

        this._onNavigationChanged.next(navigation);
    }

    proccessHiddenItems(item, userRole): FuseNavigationItem {
        ...
        if (item.hasOwnProperty("for") && item.for.indexOf(userRole) == -1) {
            item.hidden = true;
        } else {
            item.hidden= false;
        }
